...
AVStream *VideoStream::addVideoStream(AVFormatContext *oc,
                                      enum AVCodecID codec_id) // init AVFormatContext struct for output
{
    AVStream *st;
    AVCodec *codec;
    ...
    st->codec->width = 1280;
    st->codec->height = 800;
    st->codec->time_base.den = FPS; // fps
...
bool VideoStream::avWriterInit(const char *out_filename) {
    ...
    AVDictionary *opt = 0;
    av_dict_set_int(&opt, "video_track_timescale", FPS, 0); // set the fps!!!
...
void VideoStream::avWtiter(char *buffer) {
    ...
    pkt.data = mydata;
    pkt.pts = pkt.dts = frameCount * (ofmt_ctx->streams[0]->time_base.den) / ofmt_ctx->streams[0]->time_base.num / FPS;
