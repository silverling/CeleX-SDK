...
CfgSlider::CfgSlider(QWidget *parent, uint32_t min, uint32_t max, uint32_t step, uint32_t value, QWidget *widgetSlot)
    : QWidget(parent), m_uiValue(value) {
...
uint32_t CfgSlider::getValue() { return m_uiValue; }

void CfgSlider::setBiasType(std::string strType) { m_strBiasType = strType; }

std::string CfgSlider::getBiasType() { return m_strBiasType; }

void CfgSlider::setDisplayName(QString name) { m_pLabelName->setText(name + ":  "); }
...
void CfgSlider::setLineEditText(QString text) { m_pLineEditValue->setText(text); }
...
int CfgSlider::maximum() { return m_pSlider->maximum(); }
...
void CfgSlider::onSliderReleased() { emit valueChanged(m_uiValue, this); }

void CfgSlider::unblockSliderSignal() { m_pSlider->blockSignals(false); }
