...
SliderWidget::SliderWidget(QWidget *parent, uint32_t min, uint32_t max, uint32_t step, uint32_t value,
                           QWidget *widgetSlot, bool bShowVoltage)
    : QWidget(parent), m_iAddrH(-1), m_iAddrM(-1), m_iAddrL(-1), m_uiValue(value), m_pLabelVoltage(NULL) {
    ...
    m_pLineEditValue->setStyleSheet(
    ...
    connect(this, SIGNAL(valueChanged(uint32_t, SliderWidget *)), widgetSlot,
            SLOT(onValueChanged(uint32_t, SliderWidget *)));
    ...
    m_pSlider->setStyleSheet(
        ...
        "QSlider::handle:Horizontal {height: 6px; width: 10px; background-color: rgb(255, 255, 255); border-radius: "
        "3px; margin:-4 0}");
...
uint32_t SliderWidget::getValue() { return m_uiValue; }
...
void SliderWidget::setBiasAddr(int16_t addrH, int16_t addrM, int16_t addrL) {
    ...
    if (-1 == addrL) {
        ...
        if (-1 == addrM)
            ...
            m_pLabelAddr->setText("[CSR_" + QString::number(addrH) + " / CSR_" + QString::number(addrM) + " / CSR_" +
                                  QString::number(addrL) + "]:");
    }
}

std::string SliderWidget::getBiasType() { return m_strBiasType; }

void SliderWidget::setDisplayName(QString name) { m_pLabelName->setText(name + ":"); }

void SliderWidget::setDisabled(bool disable) {
    m_pSlider->setDisabled(disable);
    if (disable) {
        m_pSlider->setStyleSheet(
            "QSlider::add-page:Horizontal{background-color: rgb(120, 120, 120); height: 4px; border-radius: 3px; }"
            "QSlider::sub-page:Horizontal{background-color: rgb(120, 120, 120); height: 4px; border-radius: 3px; }"
            "QSlider::groove:Horizontal {background: transparent; height: 6px; }"
            "QSlider::handle:Horizontal {height: 6px; width: 10px; background-color: rgb(150, 150, 150); "
            "border-radius: 3px; margin:-4 0}");
    } else {
        m_pSlider->setStyleSheet(
            "QSlider::add-page:Horizontal{background-color: rgb(87, 97, 106); height: 4px; border-radius: 3px; }"
            "QSlider::sub-page:Horizontal{background-color: rgb(22, 246, 246); height: 4px; border-radius: 3px; }"
            "QSlider::groove:Horizontal {background: transparent; height: 6px; }"
            "QSlider::handle:Horizontal {height: 6px; width: 10px; background-color: rgb(255, 255, 255); "
            "border-radius: 3px; margin:-4 0}");
    }
}

void SliderWidget::setLineEditText(QString text) { m_pLineEditValue->setText(text); }
...
void SliderWidget::updateVoltage(int value) {
    if (m_pLabelVoltage) {
        double voltage;
        if (m_strBiasType == "BIAS_EVT_VL" || m_strBiasType == "BIAS_EVT_DC" || m_strBiasType == "BIAS_EVT_VH" ||
...
void SliderWidget::onSliderReleased() { emit valueChanged(m_uiValue, this); }

void SliderWidget::unblockSliderSignal() { m_pSlider->blockSignals(false); }
