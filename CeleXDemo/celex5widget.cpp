...
SensorDataObserver::SensorDataObserver(CX5SensorDataServer *sensorData, QWidget *parent)
    : QWidget(parent), m_imageMode1(CELEX5_COL, CELEX5_ROW, QImage::Format_RGB888),
      ...
      m_imageForSavePic(CELEX5_COL, CELEX5_ROW, QImage::Format_RGB888), m_bLoopModeEnabled(false), m_iPicMode(0),
      m_iLoopPicMode(0), m_bShowMultipleWindows(false), m_uiTemperature(0), m_iFPS(30), m_iPlaybackFPS(1000),
      m_uiFullFrameFPS(100), m_uiRealFullFrameFPS(0), m_bSavePics(false), m_bRecordingImages(false),
      m_lFullFrameCount(0), m_lEventFrameCount(0), m_lOpticalFrameCount(0), m_emDisplayType(Realtime),
      m_iIntervalCounter(0), m_iPicIntervalCount(0) {
...
void SensorDataObserver::setLoopModeEnabled(bool enable) { m_bLoopModeEnabled = enable; }
...
void SensorDataObserver::setFullFrameFPS(uint16_t value) { m_uiFullFrameFPS = value; }

void SensorDataObserver::setMultipleShowEnabled(bool enable) { m_bShowMultipleWindows = enable; }

bool SensorDataObserver::getMultipleShowEnable() { return m_bShowMultipleWindows; }
...
void SensorDataObserver::setSaveBmp(bool save) { m_bSavePics = save; }

bool SensorDataObserver::isSavingBmp() { return m_bSavePics; }
...
void SensorDataObserver::setPicIntervalCount(int count) { m_iPicIntervalCount = count; }
...
void SensorDataObserver::setRecordingTime(QString time) { m_qsRecordingTime = time; }
...
void SensorDataObserver::saveRecordingImage(unsigned char *pBuffer, int index) {
    ...
    if (index == 1) {
        ...
        QString name1 =
            QCoreApplication::applicationDirPath() + "/image_fullpic/" + m_qsRecordingTime + "_" + qsNum1 + ".jpg";
        char nn1[256] = {0};
        ...
        QString name2 =
            QCoreApplication::applicationDirPath() + "/image_event_count/" + m_qsRecordingTime + "_" + qsNum2 + ".jpg";
...
void SensorDataObserver::savePics(CeleX5ProcessedData *pSensorData) {
    ...
    if (pSensorData->getSensorMode() == CeleX5::Full_Picture_Mode) {
        ...
        if (pSensorData->getSensorMode() == CeleX5::Event_Off_Pixel_Timestamp_Mode) {
            folderNameList << "image_event_binary" << "image_event_count" << "image_denoised_event_binary"
                           << "image_denoised_event_count";
        ...
        for (int i = 0; i < folderNameList.size(); i++) {
            ...
            QString picName =
                g_qsCurrentRecordPath + "/" + folderNameList[i] + "/" + m_qsBinFileName + "_" + qsNum + qsFormat;
            char file_path[256] = {0};
            memcpy(file_path, picName.toStdString().c_str(), picName.size());
...
void SensorDataObserver::writeCSVData(CeleX5::CeleX5Mode sensorMode) {
    ...
    if (sensorMode == CeleX5::Event_Off_Pixel_Timestamp_Mode) {
        ...
        for (int i = 0; i < dataSize; i++) {
            g_ofWriteMat << vecEvent[i].row << ',' << vecEvent[i].col << ',' << vecEvent[i].tInPixelIncreasing << ','
                         << vecEvent[i].tOffPixelIncreasing << endl;
        }
    }
}
    ...
    if (m_bLoopModeEnabled) {
        ...
        if (m_bShowMultipleWindows) {
            ...
            if (m_pCeleX5->getSensorFixedMode() == CeleX5::Event_Off_Pixel_Timestamp_Mode ||
                ...
                painter.drawText(QRect(14, 14, 280, 30),
                                 "Event Rate: " + QString::number(m_pCeleX5->getEventRate()) + " eps");
                if (g_bStartGenerateOFFPN) {
                    painter.drawText(QRect(550, 400, 280, 30),
                                     "Fpn Progress: " + QString::number(g_dFPNProgessValue) + "%");
                }
            } else {
                painter.fillRect(QRect(10, 10, 120, 30), QBrush(Qt::blue));
                painter.drawText(QRect(14, 14, 120, 30), "FPS: " + QString::number(m_uiRealFullFrameFPS) + "/" +
                                                             QString::number(m_uiFullFrameFPS));
            }
        }
    }
...
CeleX5Widget::CeleX5Widget(QWidget *parent)
    : QWidget(parent), m_pAdSettingWidget(NULL), m_pCeleX5Cfg(NULL), m_pSettingsWidget(NULL), m_pPlaybackBg(NULL),
      m_pVersionWidget(NULL), m_pBtnPlayPause(NULL), m_emDeviceType(CeleX5::CeleX5_MIPI),
      m_emSensorFixedMode(CeleX5::Event_Off_Pixel_Timestamp_Mode), m_uiSaveBinDuration(300), m_bPlaybackPaused(false),
      m_iCurrCbBoxImageType(0), m_iCurrCbBoxLoopImageType(0), m_iPackageCountBegin(0), m_iPackageCountEnd(0) {
...
void CeleX5Widget::playback(QPushButton *pButton) {
    if ("Playback" == pButton->text()) {
        ...
        QString filePath = QFileDialog::getOpenFileName(this, "Open a bin file", QCoreApplication::applicationDirPath(),
                                                        "Bin Files (*.bin)");
        ...
        m_pBtnPlayPause->setStyleSheet(
            "QPushButton {background-color: #002F6F; background-image: url(:/images/player_pause.png); "
...
void CeleX5Widget::createButtons(QGridLayout *layout) {
    ...
    for (int i = 0; i < btnNameList.count(); ++i) {
        ...
        if ("Change FPN" == btnNameList.at(i) || "Rotate_UD" == btnNameList.at(i) ||
            "ConvertBin2CSV" == btnNameList.at(i) || "Start Recording Images" == btnNameList.at(i)) {
...
void CeleX5Widget::changeFPN() {
    QString filePath = QFileDialog::getOpenFileName(this, "Open a FPN file", QCoreApplication::applicationDirPath(),
                                                    "FPN Files (*.txt)");
...
void CeleX5Widget::showPlaybackBoard(bool show) {
    if (!m_pPlaybackBg) {
        ...
        m_pBtnReplay->setStyleSheet(
            "QPushButton {background: #002F6F; color: white; "
            "border-style: outset; border-width: 2px; border-radius: 10px; border-color: #002F6F; "
            "font: 20px Calibri; }"
            "QPushButton:pressed {background: #992F6F;}");
        ...
        pLineEditBegin->setStyleSheet(
            "QLineEdit {background: rgb(255, 255, 222); color: rgb(255, 0, 0); font: 18px Calibri}");
...
void CeleX5Widget::convertBin2CSV(QPushButton *pButton) {
    ...
    m_qstBinfilePathList = QFileDialog::getOpenFileNames(this, "Open a bin file",
                                                         QCoreApplication::applicationDirPath(), "Bin Files (*.bin)");
...
void CeleX5Widget::onButtonClicked(QAbstractButton *button) {
    ...
    if ("RESET" == button->objectName()) {
        ...
        if (CeleX5::Full_Picture_Mode == m_pCeleX5->getSensorFixedMode() || m_pCeleX5->isLoopModeEnabled()) {
    ...
    if (m_pCeleX5->getPlaybackState() == PlayFinished) {
        ...
        if (m_pSensorDataObserver->getDisplayType() == ConvertBin2Video) {
            ...
            button[0]->setStyleSheet(
                ...
                "font: 20px Calibri; }"
                "QPushButton:pressed {background: #992F6F;}");
            QMessageBox::information(NULL, "convertBin2Video", "Convert Bin to Video completely!", QMessageBox::Yes,
                                     QMessageBox::Yes);
            ...
            if (m_qstBinfilePathList.size() <= 0) {
                ...
                button[0]->setStyleSheet(
                    ...
                    "font: 20px Calibri; }"
                    "QPushButton:pressed {background: #992F6F;}");
                QMessageBox::information(NULL, "convertBin2CSV", "Convert Bin to CSV completely!", QMessageBox::Yes,
                                         QMessageBox::Yes);
...
void CeleX5Widget::onBtnPlayPauseReleased() {
    if (m_bPlaybackPaused) {
        ...
        m_pBtnPlayPause->setStyleSheet(
            "QPushButton {background-color: #992F6F; background-image: url(:/images/player_play.png); "
...
void CeleX5Widget::onBtnSavePicReleased() {
    if (m_pSensorDataObserver->isSavingBmp()) {
        ...
        m_pBtnSavePic->setStyleSheet(
            "QPushButton {background: #992F6F; color: yellow; "
            "border-style: outset; border-width: 2px; border-radius: 10px; border-color: #002F6F; "
            "font: 20px Calibri; }"
            "QPushButton:pressed {background: #992F6F; }");
    }
...
SliderWidget *CeleX5Widget::createSlider(CeleX5::CfgInfo cfgInfo, int value, QRect rect, QWidget *parent,
                                         QWidget *widgetSlot) {
