...
SettingsWidget::SettingsWidget(CeleX5 *pCeleX5, QWidget *parent)
    : QWidget(), m_pCeleX5(pCeleX5), m_pGroupBControl(NULL), m_pGroupLoop(NULL), m_pGroupAutoISP(NULL) {
...
void SettingsWidget::resizeEvent(QResizeEvent *) {
    ...
    for (int i = 0; i < cfgList1.size(); i++) {
        ...
        if (slider.size() > 0)
            slider[0]->setGeometry(left1 + 10, top1 + i * (sliderHeight + 5), m_pGroupBControl->width() - 50,
                                   sliderHeight);
    }
    ...
    for (int i = 0; i < cfgList2.size(); i++) {
        ...
        if (slider.size() > 0)
            slider[0]->setGeometry(left2 + 10, top2 + i * (sliderHeight + 5), m_pGroupBControl->width() - 50,
                                   sliderHeight);
    }
    ...
    for (int i = 0; i < cfgList3.size(); i++) {
        ...
        if (slider.size() > 0)
            slider[0]->setGeometry(left3 + 10, top3 + index * (sliderHeight + 5), m_pGroupBControl->width() - 50,
                                   sliderHeight);
        index++;
    }
}
