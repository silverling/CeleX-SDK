...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow), m_pCX5Widget(NULL) {
    ui->setupUi(this);
    this->move(10, 10);
    ...
    m_pVersionInfo->setText("Version 2.0.0");
    m_pVersionInfo->setGeometry(QRect(30, 320, 200, 20));
    m_pVersionInfo->setStyleSheet(
        "font: 20px Calibri; color: #FFFFFF;background:transparent;border-width:0;border-style:outset");

    QString strPath = QCoreApplication::applicationDirPath() + "/loading.txt";

    m_fIn.open(strPath.toLocal8Bit().data());
    string line = "";
    while (!m_fIn.eof()) {
