 ...
 */
bool XmlReader::importCeleX5Commands(std::map<std::string, std::vector<WireinCommand *>> &commandList,
                                     TiXmlDocument *pDom) {
    ...
    for (TiXmlElement *pEle = pRootEle->FirstChildElement(); nullptr != pEle; pEle = pEle->NextSiblingElement()) {
        ...
        TiXmlNode *pNode;
        WireinCommand *pCmd;
        for (TiXmlElement *pChildEle = pEle->FirstChildElement(); nullptr != pChildEle;
 ...
 */
CeleX5::CfgInfo XmlReader::getCfgInfoByName(const std::string &csrType, const std::string &name,
                                            const std::map<std::string, std::vector<CeleX5::CfgInfo>> &mapCfg) {
