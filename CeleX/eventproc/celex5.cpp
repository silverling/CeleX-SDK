...
CeleX5::CeleX5()
    : m_bLoopModeEnabled(false), m_bALSEnabled(false), m_uiClockRate(100), m_uiLastClockRate(100),
      m_iEventDataFormat(2), m_pDataToRead(nullptr), m_uiPackageCount(0), m_uiTotalPackageCount(0),
      m_bFirstReadFinished(false), m_pReadBuffer(nullptr), m_emDeviceType(CeleX5::Unknown_Devive),
      m_pCeleDriver(nullptr), m_pDataProcessor(nullptr), m_uiPackageCounter(0), m_uiPackageTDiff(0),
      m_uiPackageBeginT(0), m_bAutoISPEnabled(false), m_uiBrightness(100), m_uiThreshold(171),
      m_arrayISPThreshold{10, 20, 80}, m_arrayBrightness{95, 150, 190, 200}, m_uiAutoISPRefreshTime(80),
      m_uiISOLevel(2), m_uiOpticalFlowFrameTime(20), m_bClockAutoChanged(false), m_uiISOLevelCount(4), m_iRotateType(0),
      m_bSensorReady(false), m_bShowImagesEnabled(true), m_bAutoISPFrofileLoaded(false) {
 ...
 *  @return   :	bool : true for open sucessfully; false for failed
 */
bool CeleX5::isSensorReady() { return m_bSensorReady; }
 ...
 *  @return   :
 */
void CeleX5::getCeleXRawData(uint8_t *pData, uint32_t &length, std::time_t &timestampEnd,
 ...
 *  @return   :
 */
void CeleX5::parseCeleXRawData(uint8_t *pData, uint32_t dataSize, std::time_t timestampEnd,
 ...
 *  @return   :
 */
void CeleX5::disableFrameModule() { m_pDataProcessor->disableFrameModule(); }
 ...
 *  @return   :
 */
void CeleX5::enableFrameModule() { m_pDataProcessor->enableFrameModule(); }
 ...
 */
bool CeleX5::isFrameModuleEnabled() { return m_pDataProcessor->isFrameModuleEnabled(); }
 ...
 *  @return   :
 */
void CeleX5::disableEventStreamModule() { m_pDataProcessor->disableEventStreamModule(); }
 ...
 *  @return   :
 */
void CeleX5::enableEventStreamModule() { m_pDataProcessor->enableEventStreamModule(); }
 ...
 */
bool CeleX5::isEventStreamEnabled() { return m_pDataProcessor->isEventStreamEnabled(); }
 ...
 */
bool CeleX5::isIMUModuleEnabled() { return m_pDataProcessor->isIMUModuleEnabled(); }
 ...
 *  @return   :
 */
void CeleX5::disableEventDenoising() { m_pDataProcessor->disableEventDenoising(); }
 ...
 *  @return   :
 */
void CeleX5::enableEventDenoising() { m_pDataProcessor->enableEventDenoising(); }
 ...
 */
bool CeleX5::isEventDenoisingEnabled() { return m_pDataProcessor->isEventDenoisingEnabled(); }
 ...
 *  @return   :
 */
void CeleX5::disableFrameDenoising() { m_pDataProcessor->disableFrameDenoising(); }
 ...
 *  @return   :
 */
void CeleX5::enableFrameDenoising() { m_pDataProcessor->enableFrameDenoising(); }
 ...
 */
bool CeleX5::isFrameDenoisingEnabled() { return m_pDataProcessor->isFrameDenoisingEnabled(); }
 ...
 *  @return   :
 */
void CeleX5::disableEventCountSlice() { m_pDataProcessor->disableEventCountSlice(); }
 ...
 */
bool CeleX5::isEventCountSliceEnabled() { return m_pDataProcessor->isEventCountSliceEnabled(); }
 ...
 *  @return   :
 */
void CeleX5::disableEventOpticalFlow() { m_pDataProcessor->disableEventOpticalFlow(); }
 ...
 */
bool CeleX5::isEventOpticalFlowEnabled() { return m_pDataProcessor->isEventOpticalFlowEnabled(); }
 ...
 *  @return   :
 */
void CeleX5::getFullPicBuffer(uint8_t *buffer) { m_pDataProcessor->getFullPicBuffer(buffer); }
...
void CeleX5::getOpticalFlowPicBuffer(uint8_t *buffer, std::time_t &timestamp,
                                     OpticalFlowPicType type /* = Optical_Flow_Pic*/) {
 ...
 */
int CeleX5::getIMUData(std::vector<IMUData> &data) { return m_pDataProcessor->getIMUData(data); }
...
void CeleX5::setSensorFixedMode(CeleX5Mode mode) {
    ...
    writeRegister(AUTOISP_BRT_VALUE_H, -1, AUTOISP_BRT_VALUE_L,
                  1500); // AUTOISP_BRT_VALUE, Set initial brightness value 1500
    writeRegister(
        BIAS_BRT_I_H, -1, BIAS_BRT_I_L,
        m_uiBrightness); // BIAS_BRT_I, Override the brightness value in profile0, avoid conflict with AUTOISP profile0
    ...
    if (CeleX5::Event_In_Pixel_Timestamp_Mode == mode || CeleX5::Optical_Flow_Mode == mode ||
        CeleX5::Multi_Read_Optical_Flow_Mode == mode || CeleX5::Optical_Flow_FPN_Mode == mode) {
        // wireIn(45, 1, 0xFF);
    ...
    if (CeleX5::Optical_Flow_FPN_Mode == mode) {
        ...
        writeRegister(BIAS_RAMPP_H, -1, BIAS_RAMPP_L,
                      cfg_bias_rampn.value + (cfg_bias_rampp.value - cfg_bias_rampn.value) / 2);
    } else {
    ...
    if (CeleX5::Event_Intensity_Mode == mode || CeleX5::Full_Picture_Mode == mode) {
        setISOLevel(m_uiISOLevel);
    }
 ...
 */
CeleX5::CeleX5Mode CeleX5::getSensorFixedMode() { return m_pDataProcessor->getSensorFixedMode(); }
...
void CeleX5::setSensorLoopMode(CeleX5Mode mode, int loopNum) {
    ...
    if (getSensorLoopMode(1) == CeleX5::Event_Intensity_Mode ||
        getSensorLoopMode(1) == CeleX5::Event_In_Pixel_Timestamp_Mode ||
        getSensorLoopMode(2) == CeleX5::Event_Intensity_Mode ||
        getSensorLoopMode(2) == CeleX5::Event_In_Pixel_Timestamp_Mode ||
        getSensorLoopMode(3) == CeleX5::Event_Intensity_Mode ||
        getSensorLoopMode(3) == CeleX5::Event_In_Pixel_Timestamp_Mode) {
        if (m_uiClockRate != 70)
            setClockRate(70);
    } else {
 ...
 */
CeleX5::CeleX5Mode CeleX5::getSensorLoopMode(int loopNum) { return m_pDataProcessor->getSensorLoopMode(loopNum); }
...
void CeleX5::setLoopModeEnabled(bool enable) {
    ...
    if (enable) {
        ...
        if (CeleX5::CeleX5_MIPI == m_emDeviceType) {
            ...
            writeRegister(AUTOISP_BRT_VALUE_H, -1, AUTOISP_BRT_VALUE_L,
                          1500); // AUTOISP_BRT_VALUE, Set initial brightness value 1500
            writeRegister(BIAS_BRT_I_H, -1, BIAS_BRT_I_L,
                          m_uiBrightness); // BIAS_BRT_I, Override the brightness value in profile0, avoid conflict with
                                           // AUTOISP profile0
        }
 ...
 */
bool CeleX5::isLoopModeEnabled() { return m_bLoopModeEnabled; }
 ...
 */
bool CeleX5::setFpnFile(const std::string &fpnFile) { return m_pDataProcessor->setFpnFile(fpnFile); }
 ...
 */
uint32_t CeleX5::getClockRate() { return m_uiClockRate; }
 ...
 */
uint32_t CeleX5::getThreshold() { return m_uiThreshold; }
 ...
 */
uint32_t CeleX5::getBrightness() { return m_uiBrightness; }
 ...
 */
uint32_t CeleX5::getISOLevel() { return m_uiISOLevel; }
 ...
 */
uint32_t CeleX5::getISOLevelCount() { return m_uiISOLevelCount; }
 ...
 */
uint32_t CeleX5::getFullPicFrameTime() { return 1000 / m_uiClockRate; }
 ...
 *  @return   :
 */
void CeleX5::setEventFrameTime(uint32_t value) { m_pDataProcessor->setEventFrameTime(value, m_uiClockRate); }
 ...
 */
uint32_t CeleX5::getEventFrameTime() { return m_pDataProcessor->getEventFrameTime(); }
 ...
 */
uint32_t CeleX5::getOpticalFlowFrameTime() { return m_uiOpticalFlowFrameTime; }

void CeleX5::setEventCountSliceNum(uint32_t value) { m_pDataProcessor->setEventCountSliceNum(value); }

uint32_t CeleX5::getEventCountSliceNum() { return m_pDataProcessor->getEventCountSliceNum(); }
 ...
 *  @return   :
 */
void CeleX5::pauseSensor() { enterCFGMode(); }
 ...
 *  @return   :
 */
void CeleX5::restartSensor() { enterStartMode(); }
 ...
 *  @return   :
 */
EventShowType CeleX5::getEventShowMethod() { return m_pDataProcessor->getEventShowMethod(); }

/*
 *  @function :  setRotateType
 *  @brief    :	set rotate type
 *  @input    :	type : rotate parameter
 *					   1 for up and down flip, 2 for left and right flip, 3 for up and down left and
 *right flip
 *  @output   :
 *  @return   :
 */
 ...
 *  @return   :
 */
int CeleX5::getRotateType() { return m_iRotateType; }
 ...
 *  @return   :
 */
void CeleX5::setEventCountStepSize(uint32_t size) { m_pDataProcessor->setEventCountStep(size); }
 ...
 *  @return   :
 */
uint32_t CeleX5::getEventCountStepSize() { return m_pDataProcessor->getEventCountStep(); }
 ...
 */
int CeleX5::getEventDataFormat() { return m_iEventDataFormat; }
 ...
 *  @return   :
 */
void CeleX5::setEventFrameStartPos(uint32_t value) { m_pDataProcessor->setEventFrameStartPos(value); }
...
void CeleX5::setAutoISPEnabled(bool enable) {
    if (enable) {
        ...
        writeRegister(AUTOISP_BRT_VALUE_H, -1, AUTOISP_BRT_VALUE_L,
                      1500); // AUTOISP_BRT_VALUE, Set initial brightness value 1500
        writeRegister(BIAS_BRT_I_H, -1, BIAS_BRT_I_L, m_uiBrightness); // BIAS_BRT_I, Override the brightness value in
                                                                       // profile0, avoid conflict with AUTOISP profile0
 ...
 */
bool CeleX5::isAutoISPEnabled() { return m_bAutoISPEnabled; }
 ...
 *  @return   :	bool : true for enabled, false for diabled
 */
bool CeleX5::isALSEnabled() { return m_bALSEnabled; }
...
bool CeleX5::openBinFile(const std::string &filePath) {
    ...
    std::cout << "data_type = " << (int)m_stBinFileHeader.dataType
              ...
              << ", hour = " << (int)m_stBinFileHeader.hour << ", minute = " << (int)m_stBinFileHeader.minute
 ...
 */
uint32_t CeleX5::getTotalPackageCount() { return m_uiTotalPackageCount; }
 ...
 */
CeleX5::BinFileAttributes CeleX5::getBinFileAttributes() { return m_stBinFileHeader; }
 ...
 *  @return   :
 */
void CeleX5::play() { m_pDataProcessThread->resume(); }
 ...
 *  @return   :
 */
void CeleX5::pause() { m_pDataProcessThread->suspend(); }
 ...
 */
PlaybackState CeleX5::getPlaybackState() { return m_pDataProcessThread->getPlaybackState(); }
 ...
 *  @return   :
 */
void CeleX5::setPlaybackState(PlaybackState state) { m_pDataProcessThread->setPlaybackState(state); }
 ...
 */
CX5SensorDataServer *CeleX5::getSensorDataServer() { return m_pDataProcessor->getSensorDataServer(); }
 ...
 *  @return   :	device type
 */
CeleX5::DeviceType CeleX5::getDeviceType() { return m_emDeviceType; }
 ...
 */
uint32_t CeleX5::getFullFrameFPS() { return m_uiPackageCountPS; }
 ...
 *  @return   :	event rate
 */
uint32_t CeleX5::getEventRate() { return m_pDataProcessor->getEventRate(); }
 ...
 *  @return   :	ALS value
 */
int CeleX5::getALSValue() { return m_pCeleDriver->getALSValue(); }
 ...
 */
std::map<std::string, std::vector<CeleX5::CfgInfo>> CeleX5::getCeleX5CfgModified() { return m_mapCfgModified; }
 ...
 *  @return   :
 */
void CeleX5::saveFullPicRawData() { m_pDataProcessor->saveFullPicRawData(); }
...
void CeleX5::calDirectionAndSpeedEx(cv::Mat pBuffer, cv::Mat &speedBuffer, cv::Mat &dirBuffer) {
    ...
    int timeStampLeft, timeStampRight, timeStampUp, timeStampDown, deltaTimeLeft, deltaTimeRight, deltaTimeUp,
        deltaTimeDown;
            ...
            if (timeStampCenter < minTime || timeStampCenter > maxTime) {
                ...
                if (j <= neighborDist - 1 || j >= CELEX5_COL - neighborDist)
                    ...
                    if (pBuffer.at<uchar>(i, j + neighborDist) - pBuffer.at<uchar>(i, j - neighborDist) != 0) {
                        u = (2 * neighborDist) /
                            (pBuffer.at<uchar>(i, j + neighborDist) - pBuffer.at<uchar>(i, j - neighborDist));
                        u_min_timestamp =
                            min(pBuffer.at<uchar>(i, j + neighborDist), pBuffer.at<uchar>(i, j - neighborDist));
                    }
                }
                ...
                if (i <= neighborDist - 1 || i >= CELEX5_ROW - neighborDist)
                    ...
                    if (pBuffer.at<uchar>(i + neighborDist, j) - pBuffer.at<uchar>(i - neighborDist, j) != 0) {
                        v = (2 * neighborDist) /
                            (pBuffer.at<uchar>(i + neighborDist, j) - pBuffer.at<uchar>(i - neighborDist, j));
                        v_min_timestamp =
                            min(pBuffer.at<uchar>(i + neighborDist, j), pBuffer.at<uchar>(i - neighborDist, j));
                    }
                }
