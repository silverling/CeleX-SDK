...
DataProcessThread::DataProcessThread(const std::string &name)
    : XThread(name), m_uiPackageNo(0), m_emDeviceType(CeleX5::Unknown_Devive), m_bPlaybackBinFile(false),
      m_emPlaybackState(NoBinPlaying), m_bRecordData(false), m_bShowImagesEnabled(true) {
...
void DataProcessThread::addData(uint8_t *data, uint32_t length, std::vector<IMURawData> imuData,
...
uint32_t DataProcessThread::queueSize() { return m_queueData.size(); }

uint32_t DataProcessThread::getPackageNo() { return m_uiPackageNo; }

void DataProcessThread::setPackageNo(uint32_t no) { m_uiPackageNo = no; }

void DataProcessThread::setDeviceType(CeleX5::DeviceType type) { m_emDeviceType = type; }

void DataProcessThread::setCeleX(CeleX5 *pCeleX5) { m_pCeleX5 = pCeleX5; }

void DataProcessThread::setDataProcessor(CeleX5DataProcessor *pDataProcessor) { m_pDataProcessor = pDataProcessor; }

void DataProcessThread::setIsPlayback(bool state) { m_bPlaybackBinFile = state; }

PlaybackState DataProcessThread::getPlaybackState() { return m_emPlaybackState; }

void DataProcessThread::setPlaybackState(PlaybackState state) { m_emPlaybackState = state; }

void DataProcessThread::setRecordState(bool bRecord) { m_bRecordData = bRecord; }

void DataProcessThread::setShowImagesEnabled(bool enable) { m_bShowImagesEnabled = enable; }
