...
CeleX5DataProcessor::CeleX5DataProcessor()
    : m_uiPixelCount(0), m_uiPixelCountForEPS(0), m_uiEventNumberEPS(0), m_uiEventTCounter(0),
      m_uiEventTCounterTotal(0), m_uiEventTCounterEPS(0)
      ...
      ,
      m_uiEventCountForShow(100000), m_uiEventRowCycleCountForShow(6), m_iLastRow(-1), m_iCurrentRow(-1),
      m_uiRowCount(0), m_uiEventCountSliceNum(5), m_bIsGeneratingFPN(false), m_iFpnCalculationTimes(-1),
      m_bLoopModeEnabled(false), m_uiISOLevel(2), m_iLastRowTimestamp(-1), m_iRowTimestamp(-1), m_iMIPIDataFormat(2),
      m_emSensorFixedMode(CeleX5::Event_Off_Pixel_Timestamp_Mode), m_emSensorLoopAMode(CeleX5::Full_Picture_Mode),
      m_emSensorLoopBMode(CeleX5::Event_Off_Pixel_Timestamp_Mode), m_emSensorLoopCMode(CeleX5::Optical_Flow_Mode),
      m_uiEventFrameTime(30000), m_uiEventRowCycleCount(0), m_emEventShowType(EventShowByTime)
      ...
      ,
      m_uiCurrentEventTUnitD(100), m_uiEventFrameNo(0), m_uiEOTrampNo(1), m_uiEventCountStep(30),
      m_lCurrentEventFrameTimestamp(0), m_lLastPackageTimestamp(0), m_lFullFrameTimestamp(0), m_lEventFrameTimestamp(0),
      m_lOpticalFrameTimestamp(0), m_uiPackageTCounter(0), m_bSaveFullPicRawData(false), m_iRotateType(0),
      m_bFrameModuleEnabled(true), m_bEventStreamEnabled(true), m_bIMUModuleEnabled(true),
      m_bEventDenoisingEnabled(false), m_bFrameDenoisingEnabled(false), m_bEventCountDensityEnabled(true),
      m_bEventOpticalFlowEnabled(false), m_bStartGenerateOFFPN(false), m_iLastLoopNum(3),
      m_emLastLoopMode(CeleX5::Unknown_Mode), m_iFPNIndexForAdjustPic(1), m_uiMinInPixelTimestamp(0),
...
CeleX5DataProcessor::~CeleX5DataProcessor() {
    if (m_pCX5ProcessedData)
        delete m_pCX5ProcessedData;
    if (m_pCX5Server)
        delete m_pCX5Server;

    if (m_pEventCountBuffer)
        delete[] m_pEventCountBuffer;
    if (m_pEventADCBuffer)
        delete[] m_pEventADCBuffer;
    if (m_pLastADC)
        delete[] m_pLastADC;
    ...
    if (m_pFpnBuffer)
        delete[] m_pFpnBuffer;
    if (m_pFpnBufferOF)
        delete[] m_pFpnBufferOF;

    for (int i = 0; i < 10; i++) {
        if (m_pEventCountSlice)
            delete[] m_pEventCountSlice[i];
    }
    ...
    for (int i = 0; i < MAX_BUFFER_NUM; ++i) {
        ...
        if (m_pEventBinaryPic[i])
            delete[] m_pEventBinaryPic[i];
        if (m_pEventDenoisedBinaryPic[i])
            delete[] m_pEventDenoisedBinaryPic[i];
        if (m_pEventCountPic[i])
            delete[] m_pEventCountPic[i];
        if (m_pEventDenoisedCountPic[i])
            delete[] m_pEventDenoisedCountPic[i];
        if (m_pEventGrayPic[i])
            delete[] m_pEventGrayPic[i];
        // if (m_pEventAccumulatedPic[i]) delete[] m_pEventAccumulatedPic[i];
        if (m_pEventSuperimposedPic[i])
            delete[] m_pEventSuperimposedPic[i];
        if (m_pEventCountSlicePic[i])
            delete[] m_pEventCountSlicePic[i];

        if (m_pOpticalFlowPic[i])
            delete[] m_pOpticalFlowPic[i];
        if (m_pOpticalFlowSpeedPic[i])
            delete[] m_pOpticalFlowSpeedPic[i];
        if (m_pOpticalFlowDirectionPic[i])
            delete[] m_pOpticalFlowDirectionPic[i];
    }
    if (m_pEventAccumulatedPic)
        delete[] m_pEventAccumulatedPic;
}
 ...
 *  @return  :
 */
void CeleX5DataProcessor::getOpticalFlowPicBuffer(uint8_t *buffer, std::time_t &timestamp,
                                                  CeleX5::OpticalFlowPicType type) {
    if (type == CeleX5::OpticalFlowPic)
        memcpy(buffer, m_pOpticalFlowPic[s_uiReadFrameIndex], CELEX5_PIXELS_NUMBER);
    else if (type == CeleX5::OpticalFlowSpeedPic)
        memcpy(buffer, m_pOpticalFlowSpeedPic[s_uiReadFrameIndex], CELEX5_PIXELS_NUMBER);
...
void CeleX5DataProcessor::processMIPIData(uint8_t *pData, uint32_t dataSize, std::time_t time_stamp_end,
                                          std::vector<IMURawData> &imuData) {
    ...
    if (*(pData + dataSize - 1) ==
        0 /*|| dataSize > 500000*/) // full pic mode: package size = 1536001 = 1280 * 800 * 1.5 + 1
    {
        ...
        if (mode == 0x60 || mode == 0x80 || mode == 0xA0 || mode == 0xC0 || mode == 0xE0) {
            ...
            // cout << "CeleX5DataProcessor::processData: m_emCurrentSensorMode = " << (int)m_emCurrentSensorMode <<
            // endl;
 ...
 *  @return  :
 */
void CeleX5DataProcessor::disableFrameModule() { m_bFrameModuleEnabled = false; }
 ...
 *  @return  :
 */
void CeleX5DataProcessor::enableFrameModule() { m_bFrameModuleEnabled = true; }
 ...
 *  @return  :
 */
bool CeleX5DataProcessor::isFrameModuleEnabled() { return m_bFrameModuleEnabled; }
 ...
 *  @return  :
 */
void CeleX5DataProcessor::disableEventStreamModule() { m_bEventStreamEnabled = false; }
 ...
 *  @return  :
 */
void CeleX5DataProcessor::enableEventStreamModule() { m_bEventStreamEnabled = true; }
 ...
 *  @return  :
 */
bool CeleX5DataProcessor::isEventStreamEnabled() { return m_bEventStreamEnabled; }
 ...
 *  @return  :
 */
void CeleX5DataProcessor::disableIMUModule() { m_bIMUModuleEnabled = false; }
 ...
 *  @return  :
 */
void CeleX5DataProcessor::enableIMUModule() { m_bIMUModuleEnabled = true; }
 ...
 *  @return  :
 */
bool CeleX5DataProcessor::isIMUModuleEnabled() { return m_bIMUModuleEnabled; }

void CeleX5DataProcessor::disableEventDenoising() { m_bEventDenoisingEnabled = false; }

void CeleX5DataProcessor::enableEventDenoising() { m_bEventDenoisingEnabled = true; }

bool CeleX5DataProcessor::isEventDenoisingEnabled() { return m_bEventDenoisingEnabled; }

void CeleX5DataProcessor::disableFrameDenoising() { m_bFrameDenoisingEnabled = false; }

void CeleX5DataProcessor::enableFrameDenoising() { m_bFrameDenoisingEnabled = true; }

bool CeleX5DataProcessor::isFrameDenoisingEnabled() { return m_bFrameDenoisingEnabled; }

void CeleX5DataProcessor::disableEventCountSlice() { m_bEventCountDensityEnabled = false; }

void CeleX5DataProcessor::enableEventCountSlice() { m_bEventCountDensityEnabled = true; }

bool CeleX5DataProcessor::isEventCountSliceEnabled() { return m_bEventCountDensityEnabled; }

void CeleX5DataProcessor::disableEventOpticalFlow() { m_bEventOpticalFlowEnabled = false; }

void CeleX5DataProcessor::enableEventOpticalFlow() { m_bEventOpticalFlowEnabled = true; }

bool CeleX5DataProcessor::isEventOpticalFlowEnabled() { return m_bEventOpticalFlowEnabled; }

/*
 *  @function: processFullPicData
 *  @brief   : parse the raw full-frame data
 *  @input   : pData: a package of full-frame data
 *             dataSize: package size (default size is 1536001, if the size is not this value, there may be some thing
 * wrong with this package data) time_stamp_end: the pc timestamp when this pacakge data was received from sensor
 *  @output  :
 *  @return  :
 */
...
#pragma omp parallel num_threads(2)
    ...
    for (int i = 0; i < dataSize - 2; i += 3) {
        ...
        if (m_emCurrentSensorMode == CeleX5::Full_Picture_Mode || m_emCurrentSensorMode == CeleX5::Optical_Flow_Mode ||
 ...
 *  @brief   : parse the event format0 data, this format is not used because there is an error with the adc value output
 * from the sensor
 *  @input   : pData: a package of event format0 data
 *             dataSize: package size (default size is 357001, if the size is not this value, there may be some thing
 * wrong with this package data)
 *  @output  :
 *  @return  :
 */
void CeleX5DataProcessor::parseEventDataFormat0(uint8_t *pData, int dataSize, std::time_t timestampEnd) {
    ...
    for (int i = 0; i < dataSize - 2; i += 3) {
        ...
        } else if (dataID == 1) // b2'01: col_addr[10:0] & adc[7:0]
        {
            ...
            // cout << "format 0: " << "adc = " << adc << ", col = " << col << ", packet_format = " << packet_format <<
            // endl;
            ...
            // cout << "Format 0:  mark = " << mark << ", op_mode = " << op_mode << ", temperature = " << temperature <<
            // endl;
        }
    }
    if (m_bFrameModuleEnabled) {
 ...
 *  @brief   : parse the event format1 data, this format is used in Event Intensity mode and Event In-Pixel Timestamp
 * mode
 *  @input   : pData: a package of event format0 data
 *             dataSize: package size (default size is 357001, if the size is not this value, there may be some thing
 * wrong with this package data)
...
void CeleX5DataProcessor::parseEventDataFormat1(uint8_t *pData, int dataSize, std::time_t timestampEnd) {
    ...
    for (int i = 0; i < dataSize - 6; i += 7) {
        ...
        if (dataID1 == 2) // 2b'10
        {
            ...
            // cout << "Format 1 (Package_A): " << "row = " << m_iCurrentRow << ", row_time_stamp = " << m_iRowTimeStamp
            // << endl;
        ...
        } else if (dataID1 == 0) // 2b'00
        {
            ...
            ////cout << "Format 1 (Package_A): mark = " << mark << ", op_mode = " << op_mode << ", temperature = " <<
            /// temperature << endl;
        }
        ...
        if (dataID2 == 2) // 2b'10
        {
            ...
            // cout << "Format 1 (Package_B): " << "row = " << m_iCurrentRow << ", row_time_stamp = " << m_iRowTimeStamp
            // << endl;
            processMIPIEventTimestamp();
        ...
        } else if (dataID2 == 0) // 2b'00
        {
            ...
            // m_pCX5ProcessedData->setTemperature(temperature);
            ////cout << "Format 1 (Package_B): mark = " << mark << ", op_mode = " << op_mode << ", temperature = " <<
            /// temperature << endl;
        }
    }
 ...
 *  @brief   : parse the event format2 data, this format can only be used in Event Off-Pixel Timestamp mode because
 * there is no adc in this format
 *  @input   : pData: a package of event format0 data
 *             dataSize: package size (default size is 357001, if the size is not this value, there may be some thing
 * wrong with this package data)
 *  @output  :
...
void CeleX5DataProcessor::parseIMUData(std::time_t timestamp) {
    ...
    for (int i = 0; i < m_vectorIMURawData.size(); i++) {
        ...
        if (imuData.timestamp < timestamp /*&& imuData.time_stamp > time_stamp - 30*/) {
            ...
            // cout << "xGYROS = " << imuData.xGYROS << ", yGYROS = " << imuData.yGYROS << ", zGYROS = " <<
            // imuData.zGYROS << endl;
 ...
 */
CX5SensorDataServer *CeleX5DataProcessor::getSensorDataServer() { return m_pCX5Server; }
 ...
 */
CeleX5ProcessedData *CeleX5DataProcessor::getProcessedData() { return m_pCX5ProcessedData; }
...
void CeleX5DataProcessor::checkIfShowImage() {
    ...
    {
        if (m_uiEventRowCycleCount >= m_uiEventRowCycleCountForShow &&
            m_uiRowCount >= 100 * m_uiEventRowCycleCountForShow) {
    ...
    if (bShowImage) {
        ...
        if (0 == m_lLastPackageTimestamp) {
            ...
            m_lCurrentEventFrameTimestamp =
                m_lLastPackageTimestamp + m_uiPackageTCounter * 1024 / (m_uiCurrentEventTUnitD * 1000);
        }
...
bool CeleX5DataProcessor::createImage(std::time_t timestampEnd) {
    ...
    if (m_bLoopModeEnabled) {
        ...
        } else if (m_emSensorLoopAMode == m_emSensorLoopBMode || m_emSensorLoopAMode == m_emSensorLoopCMode ||
                   m_emSensorLoopBMode == m_emSensorLoopCMode) {
 ...
 *  @return  :
 */
CeleX5::CeleX5Mode CeleX5DataProcessor::getSensorFixedMode() { return m_emSensorFixedMode; }
 ...
 *  @return  :
 */
uint32_t CeleX5DataProcessor::getEventFrameTime() { return m_uiEventFrameTime; }
...
uint32_t CeleX5DataProcessor::getEventCountSliceNum() { return m_uiEventCountSliceNum; }
 ...
 *  @return  :
 */
EventShowType CeleX5DataProcessor::getEventShowMethod() { return m_emEventShowType; }
...
void CeleX5DataProcessor::calculateDenoisedBuffer(uint8_t *pDesBuffer1, uint8_t *pDesBuffer2, uint8_t *pSrcBuffer,
                                                  int neighbours) // 4 neighbours
{
...
void CeleX5DataProcessor::calDirectionAndSpeed(int i, int j, uint32_t *pBuffer, uint8_t *&speedBuffer,
                                               uint8_t *&dirBuffer) {
...
void CeleX5DataProcessor::calDirectionAndSpeed(uint32_t *pBuffer, uint8_t *&speedBuffer, uint8_t *&dirBuffer) {
    ...
    for (int i = 0; i < CELEX5_PIXELS_NUMBER; i++) {
        ...
        if (pBuffer[i] > 0) {
            ...
            if (pBuffer[i + markH] == 0 || pBuffer[i - markH] == 0 || pBuffer[i + markV] == 0 ||
                pBuffer[i - markV] == 0) {
                dirBuffer[i] = 0;
                // speedBuffer[i] = 0;
                continue;
 ...
 *  @return  :
 */
void CeleX5DataProcessor::setRotateType(int type) { m_iRotateType = type; }
 ...
 *  @return  :
 */
int CeleX5DataProcessor::getRotateType() { return m_iRotateType; }
 ...
 *  @return  :
 */
void CeleX5DataProcessor::setEventCountStep(uint32_t step) { m_uiEventCountStep = step; }
 ...
 *  @return  :
 */
uint32_t CeleX5DataProcessor::getEventCountStep() { return m_uiEventCountStep; }
 ...
 */
uint32_t CeleX5DataProcessor::getEventRate() { return m_uiEventNumberEPS; }

uint32_t CeleX5DataProcessor::getEventRatePerFrame() { return m_uiPixelCount; }
...
void CeleX5DataProcessor::saveOpticalFlowEvent(int col, int adc12bit, int adc8bit) {
    ...
    uint32_t tInPixelIncreasing =
        adcFPN * 7550 / 3310 + tRampNo * 7550; //(adcFPN+tRampNo*3310)*7750/3310, 3310*2.281=7750¦Ìs
...
void CeleX5DataProcessor::calEventCountSlice(uint8_t *pEventCountSliceBuffer) {
    ...
    if (m_uiEventCountSliceNum == 5) {
        for (int i = 0; i < CELEX5_PIXELS_NUMBER; i++) {
            index = getCurrentIndex(i);
            value = m_pEventCountBuffer[i] * 10 + m_pEventCountSlice[0][i] * 9 + m_pEventCountSlice[1][i] * 8 +
        ...
        for (int i = 0; i < CELEX5_PIXELS_NUMBER; i++) {
            value = m_pEventCountBuffer[i] * 10 + m_pEventCountSlice[0][i] * 9 + m_pEventCountSlice[1][i] * 8 +
                    m_pEventCountSlice[2][i] * 7 + m_pEventCountSlice[3][i] * 6 + m_pEventCountSlice[4][i] * 5;
        ...
        for (int i = 0; i < CELEX5_PIXELS_NUMBER; i++) {
            value = m_pEventCountBuffer[i] * 10 + m_pEventCountSlice[0][i] * 9 + m_pEventCountSlice[1][i] * 8 +
                    m_pEventCountSlice[2][i] * 7 + m_pEventCountSlice[3][i] * 6 + m_pEventCountSlice[4][i] * 5 +
                    m_pEventCountSlice[5][i] * 4 + m_pEventCountSlice[6][i] * 3;
