...
void USBInterface::worker(void) {
    while (m_bRunning) {
        if (g_bTransferError) {
            ...
            // open video stream
            uint8_t video_start[] = {0x00, 0x00, 0x01, 0x02, 0x0A, 0x8B, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x1C, 0x00, 0x00, 0x90, 0x00, 0x00};
                        ...
                        for (int k = 0; k < config->interface[j].num_altsetting; k++) {
                            if (config->interface[j].altsetting[k].bInterfaceClass == LIBUSB_CLASS_VIDEO) {
                                ...
                                for (int l = 0; l < config->interface[j].altsetting[k].bNumEndpoints; l++) {
                                    if (!bFindVideo && (config->interface[j].altsetting[k].endpoint[l].bmAttributes &
                                                        0x03) == transMode) // find endpoint: video
                                    {
                                        m_iVideoEndpointAddress =
                                    ...
                                    if (!bFindIMU && (config->interface[j].altsetting[k].endpoint[l].bmAttributes &
                                                      0x03) == LIBUSB_TRANSFER_TYPE_INTERRUPT) // find endpoint: imu
                                    {
                                        m_iIMUEndpointAddress =
                                            config->interface[j].altsetting[k].endpoint[l].bEndpointAddress;
                                        // libusb_free_config_descriptor(config);
                                        // return true;
                                        printf("------------------- find imu!\n");
        ...
        if (usbGetInterface(vid, pid, transMode) == true) {
            ...
            if (m_pDeviceHandle) {
                ...
                for (; i < m_vecInterfaceNumberList.size(); i++) {
                    ...
                    if ((Ret == LIBUSB_SUCCESS) || (Ret == LIBUSB_ERROR_NOT_SUPPORTED) ||
 ...
 *             buffer: a suitably-sized data buffer for either input or output (depending on direction bits within
 * bmRequestType) len: the length field for the setup packet. The data buffer should be at least this size.
 *  @output  :
 *  @return  : true is successful to write sensor serial number
 */
bool USBInterface::usbControl(uint8_t requestType, uint8_t request, uint16_t wValue, uint16_t index, uint8_t *buffer,
                              uint16_t len) {
    if (m_pDeviceHandle) {
        int ret =
            libusb_control_transfer(m_pDeviceHandle, requestType, request, wValue, index, buffer, len, USB_TIMEOUT);
        if (ret == len) {
            return true;
        }
...
bool USBInterface::usbAllocInterruptTransfer(void) {
    ...
    for (i = 0; i < MAX_URB_NUMBER_IMU; i++) {
        m_pInterruptTransfer[i] =
            alloc_interrupt_transfer(m_pDeviceHandle, m_iIMUEndpointAddress, m_uiInterruptBuffer[i]);
