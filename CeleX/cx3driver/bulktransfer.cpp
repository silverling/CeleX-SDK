...
libusb_transfer *alloc_bulk_transfer(libusb_device_handle *device_handle, uint8_t address, uint8_t *buffer) {
    if (device_handle) {
        ...
        if (xfr) {
            libusb_fill_bulk_transfer(xfr, device_handle,
                                      address, // Endpoint ID
                                      buffer, MAX_ELEMENT_BUFFER_SIZE, callbackUSBTransferCompleted, nullptr, 0);
...
libusb_transfer *alloc_interrupt_transfer(libusb_device_handle *device_handle, uint8_t address, uint8_t *buffer) {
    if (device_handle) {
        libusb_transfer *xfr = libusb_alloc_transfer(0);
        if (xfr) {
            libusb_fill_interrupt_transfer(xfr, device_handle,
                                           address, // Endpoint ID
                                           buffer, 32, cbUSBInterruptTransferCompleted, nullptr, 0);
            if (submit_bulk_transfer(xfr) == true)
                return xfr;
        }
    }
